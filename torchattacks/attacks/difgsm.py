...
class DIFGSM_Eva(Attack):
    """
    DI2-FGSM that returns all step-wise predictions for external evaluation.
    ...
    def __init__(
        ...
        super().__init__("DIFGSM_Eva", model)
        self.eps = eps
        self.steps = steps
        self.decay = decay
        self.alpha = alpha
    ...
    def input_diversity(self, x):
        ...
        h_rem = img_resize - rnd
        w_rem = img_resize - rnd
        pad_top = torch.randint(low=0, high=int(h_rem.item()), size=(1,), dtype=torch.int32)
        pad_bottom = h_rem - pad_top
        pad_left = torch.randint(low=0, high=int(w_rem.item()), size=(1,), dtype=torch.int32)
        pad_right = w_rem - pad_left

        padded = F.pad(
            rescaled,
            [int(pad_left.item()), int(pad_right.item()), int(pad_top.item()), int(pad_bottom.item())],
            value=0,
        )
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            with torch.no_grad():
                preds = torch.argmax(self.get_logits(adv_images), dim=1)
                step_preds_list.append(preds.detach().cpu())

        return adv_images, step_preds_list